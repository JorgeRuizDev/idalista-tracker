...
1. Detect all saved searches from the user's Idealista account (stored locally in Chrome Storage)
...
**Key Design Decision**: Saved searches are stored locally in the extension's Chrome Storage, not in the backend. This keeps the extension self-contained and avoids unnecessary backend coupling.
...
**Note**: `SavedSearch` is stored locally in the extension's Chrome Storage, not in the backend. This allows the extension to work independently and avoids sync issues.
...
- `src/api/routes/searches.py` - Saved searches are now stored locally in the extension
...
1. Create new tables (CrawlSession, PropertyVisibility, PropertyChange)
2. Create association table
3. Add columns to Property table
4. Backfill data where needed

**Note**: Remove SavedSearch table creation - searches are stored locally in extension.
...
**Decision**: Chrome Storage API for all extension state

**Stored Locally**:
- Extension configuration (server URL, delays)
- Saved searches (extracted from Idealista page)
- Crawl state (progress, session info)
...
- No external dependencies for search configuration
- Self-contained extension architecture
...
**Decision**: Extend existing models with history tracking, keep searches local

**Backend Key Additions**:
...
**Extension Storage** (local only):
- `SavedSearch` - stored in Chrome Storage API, not synced to backend
...
- **Saved searches stored locally**: Avoids unnecessary backend coupling, allows extension to work independently, simpler architecture
